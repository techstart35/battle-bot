...
func GetRandomSoloBattleTmpl(loser string, seed int) string {
	var tmpl = []string{
		...
		fmt.Sprintf("💥｜**%s** はバナナの皮で滑って気絶🍌", loser),
		fmt.Sprintf("💥｜**%s** は迷子で行方不明に。", loser),
		fmt.Sprintf("💥｜**%s** は腐った肉を食べて腹痛。戦闘不能。", loser),
		fmt.Sprintf("💥｜**%s** はご飯の食べ過ぎで破裂。", loser),
		fmt.Sprintf("💥｜**%s** は豆腐の角に頭をぶつけて即死。", loser),
		...
		fmt.Sprintf("💥｜**%s** は賞味期限切れの生卵を食す。腹を壊した。", loser),
		fmt.Sprintf("💥｜**%s** は快速特急に乗ってしまい下車するはずの駅で降りられず。脱落。", loser),
		fmt.Sprintf("💥｜**%s** はランブルの勝ち方を解明すべくアマゾンの奥地へ向かった。", loser),
		fmt.Sprintf("💥｜**%s** は木登りをしていたが、足を滑らせ滑落。", loser),
		fmt.Sprintf("💥｜**%s** はつま先立ちで歩いていたため、足の指を骨折。", loser),
		fmt.Sprintf("💥｜**%s** は車に轢かれそうな子供を助けて代わりに事故死。", loser),
		fmt.Sprintf("💥｜**%s** は神隠しにあった。", loser),
		fmt.Sprintf("💥｜**%s** は料理中に指を切って出血死。", loser),
		fmt.Sprintf("💥｜**%s** はつまづいて頭蓋骨粉砕した。", loser),
		fmt.Sprintf("💥｜**%s** はあつあつのおでんを食べてショック死した。", loser),
		fmt.Sprintf("💥｜**%s** は奥さんに送るはずのLINEを同僚に誤爆し、戦いどころではない。", loser),
	}
