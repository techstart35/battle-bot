...
// バトル構造体を作成します
func NewTanakaBattleApp(app *app.App) *BattleApp {
	a := &BattleApp{}
	a.App = app
...
func (a *BattleApp) TanakaBattle(guildID, channelID, authorID string, input []string) error {
...
`
	...
	embedInfo := &discordgo.MessageEmbed{
		Title:       "【TANAKA】Battle Royaleが起動されました",
		Description: msg,
...
`
	...
	embedInfo := &discordgo.MessageEmbed{
		Title:       "【TANAKA】キャンセル処理が正常に終了しました",
		Description: fmt.Sprintf(MessageTmpl, guildName),
		Color:       shared.ColorBlue,
		Timestamp:   shared.GetNowTimeStamp(),
	}
...
`
	...
	embedInfo := &discordgo.MessageEmbed{
		Title:       "【TANAKA】NoEntryで正常に終了しました",
		Description: fmt.Sprintf(MessageTmpl, guildName),
		Color:       shared.ColorBlue,
		Timestamp:   shared.GetNowTimeStamp(),
	}
